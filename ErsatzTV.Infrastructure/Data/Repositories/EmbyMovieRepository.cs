...
public class EmbyMovieRepository : IEmbyMovieRepository
{
    ...
    public async Task<List<int>> FlagFileNotFound(EmbyLibrary library, List<string> embyMovieItemIds)
    {
        if (embyMovieItemIds.Count == 0)
        {
            return new List<int>();
        }
        ...
        List<int> ids = await dbContext.Connection.QueryAsync<int>(
                ...
                new { LibraryId = library.Id, MovieItemIds = embyMovieItemIds })
    ...
    public async Task<Either<BaseError, MediaItemScanResult<EmbyMovie>>> GetOrAdd(EmbyLibrary library, EmbyMovie item)
    {
        ...
        return await AddEmbyMovie(dbContext, library, item);
    }
    ...
    private async Task<Either<BaseError, MediaItemScanResult<EmbyMovie>>> AddEmbyMovie(
        TvContext dbContext,
        EmbyLibrary library,
        EmbyMovie movie)
    {
